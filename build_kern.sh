...
build_kernel()
{
	...
	make -j$(nproc) -C $(pwd) O=$OUT_DIR $KERNEL_MAKE_ENV ARCH=arm64 CROSS_COMPILE=$BUILD_CROSS_COMPILE CC=$KERNEL_LLVM_BIN CLANG_TRIPLE=$CLANG_TRIPLE vendor/nx659j_defconfig vendor/nx659j.config
	...
	if [ -e $OUT_DIR/arch/arm64/boot/Image.gz-dtb ]; then
		...
		cat ${DTS_DIR}/qcom/*.dtb > $OUT_DIR/dtb
	fi
}
...
anykernel3()
{
	...
	if [ -e $OUT_DIR/arch/arm64/boot/Image.gz-dtb ]; then
		cp $OUT_DIR/arch/arm64/boot/Image.gz-dtb $PARENT_DIR/AnyKernel3/Image.gz-dtb
		cp $OUT_DIR/dtb $PARENT_DIR/AnyKernel3/dtb
		...
		zip -r9 $PARENT_DIR/nx659j_kernel_`cat $OUT_DIR/include/config/kernel.release`_`date '+%Y_%m_%d'`.zip * -x .git README.md *placeholder
		cd $DIR
	fi
}
